...
def recursively_extract(name, obj):
    if isinstance(obj, h5py.Dataset):
        ...
        if layer_name not in visited_layers:
            ...
            elif 'activation' in layer_name or 'gelu' in layer_name or 'relu' in layer_name or 'softmax' in layer_name:
...
def array_to_c_array(name, arr):
    if arr.ndim == 2:
        out = f"const data_t {name}[{arr.shape[1]}][{arr.shape[0]}] = {{\n"
        arr = arr.T
        for row in arr:
...
# 儲存 weights.h
with open(os.path.join(output_dir, 'weights.h'), 'w') as f_out:
...
# -------------------------------------------
# 寫出 config.h
with open(os.path.join(output_dir, 'config.h'), 'w') as f_out:
...
# -------------------------------------------
# 寫出 common.h
input_dim = list(weights_dict.values())[0].shape[0]
output_dim = list(weights_dict.values())[-1].shape[1]

with open(os.path.join(output_dir, 'common.h'), 'w') as f_out:
    f_out.write("#ifndef COMMON_H\n")
    f_out.write("#define COMMON_H\n\n")
    f_out.write('#include <ap_fixed.h>\n')
    f_out.write('#include <hls_math.h>\n')
    f_out.write(f"#define DIM {input_dim}\n")
    f_out.write(f"#define FF_DIM {output_dim}\n\n")
    f_out.write("typedef ap_fixed<16,6> data_t;\n\n")
    f_out.write("#endif\n")

print("Saved common.h")
